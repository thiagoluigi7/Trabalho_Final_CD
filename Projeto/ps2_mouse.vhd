...
ENTITY ps2_mouse IS
	GENERIC(
			clk_freq							:	INTEGER := 25_175_000;	--system clock frequency in Hz
				...
				WHEN stream =>
					IF(ps2_code_new_prev = '0' AND ps2_code_new = '1') THEN								--new PS/2 code received
						...
						IF(packet_byte = 2) THEN
							mouse_data_int(23 DOWNTO 16) <= ps2_code;					--store new mouse data byte
						ELSIF(packet_byte = 1) THEN
							mouse_data_int(15 DOWNTO 8) <= ps2_code;
						ELSIF(packet_byte = 0) THEN
							mouse_data_int(7 DOWNTO 0) <= ps2_code;
						END IF;
